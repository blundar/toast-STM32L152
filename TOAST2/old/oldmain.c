...
int adc_ad8495_conv(adcsample_t reading) {
	int temperature;
...
//    adcsample_t avg_ch1, avg_ch2;
    ...
    pwmEnableChannelI(&PWMD4, 0, PWM_FRACTION_TO_WIDTH(&PWMD4, 2048, avg_ch1));
    pwmEnableChannelI(&PWMD4, 1, PWM_FRACTION_TO_WIDTH(&PWMD4, 2048, avg_ch2));
...
int main(void) {
  ...
  //Light em up, fixed duty
//    pwmEnableChannel(&PWMD4, 0, 1250);
//    pwmEnableChannel(&PWMD4, 1, 625);
  ...
  while (TRUE) {
    ...
    temp_read1 = adc_ad8495_conv(avg_ch2);
    chprintf((BaseChannel *)&SD1, "ch1: %d ch2: %d deg: %d\r\n", avg_ch1, avg_ch2,temp_read1);
  }
}
